...
local create_window = function()
	if type == 1 then
		...
		vim.cmd("vertical new errorlist | vertical resize"
		..options.size)
	end
end
...
function M.run_program(args)
	...
	if not M.filetypes[ft] or (
		...
		print(
			"No compilers or interpreters config found for filetype '"
			..ft.."'")
		return
	end
	...
	if options.save then vim.cmd("silent! w") end
